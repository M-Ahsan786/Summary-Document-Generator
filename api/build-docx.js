// api/build-docx.js — Builds final docx from merged summary data
import {
...
export default async function handler(req, res) {
    ...
    try {
        const { summary, courseName } = req.body;
        if (!summary || !courseName) return res.status(400).json({ error: 'summary and courseName required' });

        const docBuffer = await buildDocx(summary, courseName);
        const base64 = docBuffer.toString('base64');
        const filename = `${courseName.replace(/[^a-z0-9]/gi, '_')}_Summary.docx`;

        return res.status(200).json({ ok: true, docx: base64, filename, summary });
    } catch (err) {
