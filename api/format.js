...
// Receives a base64 docx, extracts content, rebuilds with our standard format
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Step 1: Extract text content from uploaded docx
        const inputBuffer = Buffer.from(docxBase64, 'base64');
        const extracted = await extractDocxContent(inputBuffer);

        // Step 2: Parse extracted content into structured data
        const structured = parseStructure(extracted);

        // Step 3: Build new formatted docx
        const outputBuffer = await buildFormattedDocx(structured);
...
// ═══════════════════════════════════════════════
async function buildFormattedDocx(data) {
        ...
        for (const extra of (lab.extra || [])) {
            children.push(
                new Paragraph({
                    ...
                    indent: { left: 400 },
                    children: [
                        new TextRun({ text: extra, size: 24, font: 'Times New Roman', color: DARK_GREY })
                    ]
                })
            );
        }
...
// Patch: offsetFrom="page" + footer logo fix
